...
describe('HomeGroupService', () => {
  ...
  const service = new HomeGroupService({
    homeGroup: { findUnique, create, update, delete: remove },
  ...
  it('creates a group without a leader', async () => {
    ...
    await expect(service.create({ name: ' Винники, четвер ' })).resolves.toMatchObject({
      name: 'Винники, четвер',
      leader: null,
      peopleCount: 0,
    });
    expect(create).toHaveBeenCalledWith(
      expect.objectContaining({ data: { name: 'Винники, четвер', address: null } }),
    );
  });
